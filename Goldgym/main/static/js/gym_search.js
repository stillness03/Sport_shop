document.addEventListener('DOMContentLoaded', function() {
    ...
    gymCards.forEach(card => {
        ...
        const gymAddress = card.querySelector('.card-text').textContent.toLowerCase();
        const cityName = card.dataset.city?.toLowerCase() || '';

        const isMatch = gymName.includes(searchTerm) || gymAddress.includes(searchTerm) || cityName.includes(searchTerm);
        card.style.display = isMatch ? 'block' : 'none';
    });

    cityHeaders.forEach(header => {
        const cityCards = header.nextElementSibling.querySelectorAll('.gym-card');
        const hasVisible = Array.from(cityCards).some(card => card.style.display !== 'none');
