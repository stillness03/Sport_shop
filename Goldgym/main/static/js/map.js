...
function updateVisibleGyms(gyms) {
    ...
    const sortedCities = Object.keys(cities).sort((a, b) => {
        ...
        const distA = L.latLng(center).distanceTo(L.latLng(gymA.latitude, gymA.longitude));
        const distB = L.latLng(center).distanceTo(L.latLng(gymB.latitude, gymB.longitude));
        return distA - distB;
    });
                ...
                <div class="cards-body">
                    <div class="d-flex justify-content-between align-items-start mb-1">
                        ...
                        <div class="text-end">
                            ...
                            <span class="text-muted small">${Math.round(center.distanceTo(L.latLng(gym.latitude, gym.longitude)) / 1000)} km away</span>
                        </div>
                    </div>
                    <p class="card-text text-muted">${gym.address}</p>
                    <button class="btn btn-sm btn-outline-primary view-gym-btn" data-gym-id="${gym.id}">VIEW DETAILS →</button>
                </div>
            `;
